...
namespace Comision.Helper
{
  ...
  this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TEnum>> expression, object htmlAttributes, string placeholderName = "", bool Issplit = false, char splitchar = '@', int splitindex = 1)
        {
            ...
            var items =
                values.Select(
                   ...
                   new SelectListItem
                   {
                       ...
                       Text = Issplit == true ? Useful.GetEnumDescription(value).Split(splitchar)[splitindex] : Useful.GetEnumDescription(value),
            ...
            {
                if (!items.Where(i => i.Text == string.Empty).Any())
                    items.Insert(0, new SelectListItem { Selected = false, Text = placeholderName, Value = string.Empty });

                if (items.Any() || items[0].Text == string.Empty)
                {
                    items[0].Value = "";
                    items[0].Text = placeholderName;
                }
            }
       ...
       public static MvcHtmlString EnumDropDownList<TEnumType>(this HtmlHelper htmlHelper, string name, TEnumType value)
        ...
        public static MvcHtmlString EnumDropDownListFor<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, IDictionary<string, object> htmlAttributes = null) where TModel : class
        {
            ...
            var name = ExpressionHelper.GetExpressionText(expression);
            var fullName = htmlHelper.ViewContext.ViewData.TemplateInfo.GetFullHtmlFieldName(name).Replace("_", " ");
            ModelState currentValueInModelState;
            var couldGetValueFromModelState = htmlHelper.ViewData.ModelState.TryGetValue(fullName, out currentValueInModelState);
            object selectedValue = null;
